...
export default function DownloadModal({
  ...
  const [selectedHistoryPeriod, setSelectedHistoryPeriod] = useState(availableHistoryPeriods[0] || '');
  ...
  const generateCSVData = () => {
    const municipalities = selectedMunicipalities.length > 0 
  ...
  const handleDownload = async () => {
    ...
    link.setAttribute('download', 
      `medication-demand-${selectedMedication.replace(/\s+/g, '-').toLowerCase()}-${predictionType}-${new Date().toISOString().split('T')[0]}.csv`
    );
...
const [localPredictionType, setLocalPredictionType] = useState(predictionType);


  return (
    <div className="fixed inset-0 bg-opacity-40 backdrop-blur-sm  bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative animate-in fade-in duration-200">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Configure your data export settings</p>
            </div>
          </div>
          <button
            onClick={handleClose}
  ...
  <div className="flex items-center gap-2 mb-2">
    ...
    <span className="text-sm font-medium text-blue-900">Customize Selection</span>
  </div>

  <div className="grid grid-cols-2 gap-4 text-sm">
    {/* Medication dropdown */}
    <div>
      <label className="text-blue-700 font-medium block mb-1">Medication</label>
      <select
        value={localMedication}
        onChange={(e) => {
          setLocalMedication(e.target.value);
          onMedicationChange?.(e.target.value); // if external callback needed
        }}
    ...
    <div>
      <label className="text-blue-700 font-medium block mb-1">Prediction Type</label>
      <select
      ...
      >
        ...
        <option value="monthly">Monthly</option>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-medium text-gray-700">Municipalities</label>
              </div>
              <button
            ...
            <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-3">
              <div className="grid grid-cols-2 gap-2">
                {availableMunicipalities.map((municipality) => (
                  <label key={municipality} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                    <input
                      type="checkbox"
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <label className="text-sm font-medium text-gray-700">History Period</label>
            </div>
            <select
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Include confidence levels</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Include outlier detection</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
          <div className="text-sm text-gray-600">
            Export format: CSV • File size: ~{Math.ceil((selectedMunicipalities.length || availableMunicipalities.length) * 0.1)}KB
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleClose}
